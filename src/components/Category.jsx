...
export default function Category() {
    ...
    return (
        ...
        <div onClick={sliderActions} id="places-category-box" className="w-[98%] h-[100px] py-1 px-4 rounded-xl z-[1000] bg-white  sticky top-[75px]">
            <div className="w-full h-full overflow-x-scroll ul-box">
                <ul id="places-category-ul" className="w-auto py-3 px-1 flex gap-6 m-0">
                    {mainCategories?.map(item => {
                        ...
                        return (
                            <li onClick={() => { handleCatCookie(item.slug) }} className="min-w-[92px] bg-gray-50 rounded-3xl p-2" key={item.id}>
                                <a href={cookie['cities'] !== undefined && cookie['cities'].length > 0 ? `/s/iran/${item.slug}?cities=${cookieCitiesInUrl}` : `/s/iran/${item.slug}`} className='w-full h-full cursor-pointer flex flex-col gap-3 justify-center items-center'>
