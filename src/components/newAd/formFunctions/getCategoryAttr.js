      ...
      for (let key in newAdStorageValue) {
        ...
        adsCategoriesList?.find(item => {
          ...
          item?.attributes?.find(mainAttr => {
            if (mainAttr.id == [key]) { catAttrTitle = mainAttr.title, catAttrType = mainAttr.type }
          })
            ...
            itemCh.id === newAdStorageValue?.category.id &&
              (
                ...
                itemCh.attributes.find(itemAttr => {
                  ...
                  if (itemAttr.id == [key]) { catAttrTitle = itemAttr.title, catAttrType = itemAttr.type }
                }))
          })
  
        })
  
        JSON.stringify(+key).length != 4 && catAttr.push({ "name": catAttrTitle, "id": key, "type": catAttrType, "value": newAdStorageValue[key] })
      }
    }
